...
def main(input_dim: int, num_tasks: int, seq_length: int, vs: List[float],
         ...
         simulation: int, ode: int, logdir: str, seeds: int, args: dict):
    ...
    for seed in range(seeds):
        ...
        if ode:
            ...
            for i in range(max_iters[0]):
                ...
                if ode_solver.overlap_task[0] >= threshold:
            ...
            for i in range(max_iters[1]):
                ...
                if ode_solver.overlap >= threshold:
        ...
        if simulation:
            ...
            for i in range(max_iters[0]):
                ...
                if np.all(sim_solver.overlap_task >= threshold):
            ...
            for i in range(max_iters[1]):
                sim_solver.step()
                if np.all(sim_solver.overlap >= threshold):
