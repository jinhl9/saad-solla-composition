...
def main(input_dim: int, num_tasks: int, seq_length: int, lr_ws: List[float],
    ...
    for seed in range(seeds):
        ...
        dataloader = data.iid.TransientRLTask(batch_size = 1, seq_len = [seq_length, seq_length, seq_length,seq_length], input_dim = input_dim, identical = identical)
        simulator = solver.TwoPhaseContextSolver(teachers=teachers, students = students, context_teacher=context_teacher, context_student=context_student, dataloaders = [dataloader, dataloader, dataloader], logdir = None, identical=identical)
        simulator.train(nums_iter = nums_iter, lrs = [(lr_ws[0], 0),(lr_ws[1], lr_v)], update_frequency=update_frequency)
        joblib.dump(ode_solver.history, os.path.join(log_folder, f'ode_{seed}.jl'))
        joblib.dump(simulator.history, os.path.join(log_folder, f'simulator_{seed}.jl'))

        weights={i: {'WS': student.layers[0].weight.data, 'WT': teacher.layers[0].weight.data}  for i, (student, teacher) in enumerate(zip(simulator.students, simulator.teachers))}
        weights['vs'] = simulator.context_student.layers[0].weight.data
        joblib.dump(weights, os.path.join(log_folder, f'simulator_{seed}_weights.jl'))
